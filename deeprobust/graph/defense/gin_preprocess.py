...
from deeprobust.graph.defense import GIN
...
class GINSVD(GIN):
    """GINSVD is a 2 Layer Graph Convolutional Network with Truncated SVD as
    ...
    weight_decay : float
        weight decay coefficient (l2 normalization) for GIN. When `with_relu` is True, `weight_decay` will be set to 0.
    with_relu : bool
        whether to use relu activation function. If False, GIN will be linearized.
    with_bias: bool
        whether to include bias term in GIN weights.
    device: str
        'cpu' or 'cuda'.

    Examples
    --------
	We can first load dataset and then train GINSVD.

    >>> from deeprobust.graph.data import PrePtbDataset, Dataset
    >>> from deeprobust.graph.defense import GINSVD
    ...
    >>> # train defense model
    >>> model = GINSVD(nfeat=features.shape[1],
              nhid=16,
    ...
    def __init__(self, nfeat, nhid, nclass, dropout=0.5, lr=0.01, weight_decay=5e-4, with_relu=True, with_bias=True, device='cpu'):
        ...
        super(GINSVD, self).__init__(nfeat, nhid, nclass, dropout, lr, weight_decay, with_relu, with_bias, device=device)
        self.device = device
        self.k = None

    def fit(self, features, adj, labels, idx_train, idx_val=None, k=20, train_iters=200, initialize=True, verbose=True, **kwargs):
        """First perform rank-k approximation of adjacency matrix via
        truncated SVD, and then train the gin model on the processed graph,
        ...
        idx_val :
            node validation indices. If not given (None), GIN training process will not adpot early stopping
        k : int
    ...
    def predict(self, features=None, adj=None):
        ...
        -------
        torch.FloatTensor
            output (log probabilities) of GINSVD
        """
...
class GINJaccard(GIN):
    """GINJaccard first preprocesses input graph via droppining dissimilar
    edges and train a GIN based on the processed graph. See more details in
    ...
    Examples
    --------
	We can first load dataset and then train GINJaccard.

    >>> from deeprobust.graph.data import PrePtbDataset, Dataset
    >>> from deeprobust.graph.defense import GINJaccard
    >>> # load clean graph data
    >>> data = Dataset(root='/tmp/', name='cora', seed=15)
    >>> adj, features, labels = data.adj, data.features, data.labels
    >>> idx_train, idx_val, idx_test = data.idx_train, data.idx_val, data.idx_test
    ...
    >>> perturbed_adj = perturbed_data.adj
    >>> # train defense model
    >>> model = GINJaccard(nfeat=features.shape[1],
              nhid=16,
              nclass=labels.max().item() + 1,
              dropout=0.5, device='cpu').to('cpu')
    ...
    def __init__(self, nfeat, nhid, nclass, binary_feature=True, dropout=0.5, lr=0.01, weight_decay=5e-4, with_relu=True, with_bias=True, device='cpu'):
        ...
        super(GINJaccard, self).__init__(nfeat, nhid, nclass, dropout, lr, weight_decay, with_relu, with_bias, device=device)
        self.device = device
    ...
    def fit(self, features, adj, labels, idx_train, idx_val=None, threshold=0.01, train_iters=200, initialize=True, verbose=True, **kwargs):
        ...
        threshold and then train the gin model on the processed graph.
    ...
    def predict(self, features=None, adj=None):
        ...
        -------
        torch.FloatTensor
            output (log probabilities) of GINJaccard
        """
