...
def normalize_tensor_adj(adj_tensor):
    # Sum up the adjacency tensor along the last dimension (assuming it represents edges)
    degree = torch.sum(adj_tensor, dim=-1, keepdim=True)
    # Calculate the degree matrix's inverse square root
    degree_inv_sqrt = torch.pow(degree, -0.5)
    # Replace infinities with zeros, if any (avoid division by infinity)
    degree_inv_sqrt[degree_inv_sqrt == float('inf')] = 0
    # Apply the degree matrix's inverse square root to normalize the adjacency tensor
    normalized_adj = degree_inv_sqrt * adj_tensor * degree_inv_sqrt.transpose(-2, -1)
...
if __name__ == '__main__':
    ...
    if args.dataset == "CS":
        ...
        dataset = Planetoid("/home/ubuntu/lj/NoisyGNN-main/Datasets", args.dataset,
                                                transform=T.NormalizeFeatures())
