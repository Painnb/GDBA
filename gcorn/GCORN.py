...
class ConvClass(nn.Module):
    ...
    def csr_to_tensor(self,x):
        ...
        # 创建稀疏张量
        x = torch.sparse.FloatTensor(indices_tensor, values_tensor, torch.Size(coo.shape)).cuda()

        return x

    def forward(self, x, adj):
        from .matrix_ortho import scale_values
        from .matrix_ortho import orthonormalize_weights
        scaling = scale_values(self.weight.data).cuda()
        ...
        if isinstance(adj, csr_matrix):
            adj = self.csr_to_tensor(adj)
        if isinstance(x, csr_matrix):
            x = self.csr_to_tensor(x)
